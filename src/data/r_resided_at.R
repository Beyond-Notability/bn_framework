...
bn_dates_all <-
  ...
  mutate(residence = case_when(
  	propertyLabel=="resided at" ~ "resided at",
  	!is.na(propertyLabel) ~ "other"
  )) 
...
bn_dates_ages <-
  bn_dates_all |> 
  filter(year<2020) |>
  inner_join(bn_women_list_deduped  |> 
  	filter(!is.na(bn_dob_yr)) |> select(bn_id, bn_dob_yr), by="bn_id") |>
  mutate(age = year-bn_dob_yr) |>
  arrange(bn_id, age) 

# dotplot definitely better.
bn_dates_ages_not_dob <-
	bn_dates_ages |>
	filter(propertyLabel != "date of birth")
...
# and do you need the left join for residence ... shouldn't do?
dates_resided_late <-
bn_dates_ages |>
...
# this isn't all.... 
dates_resided_early_late_other <-
bind_rows(
  dates_resided_early |> mutate(group="early"),
  dates_resided_late |> mutate(group="late") ,
  dates_resided_other |> mutate(group="other")
) 